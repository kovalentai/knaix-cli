...
///
/// This was a bool, which could say "print" but not how, so the REPL had to
/// choose between streaming raw and rendering markdown at the end -- and chose
/// the end, holding an answer that was already arriving. The third case makes
/// the choice a real one.
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum Echo {
    /// Accumulate silently; the caller prints the finished answer, or nothing.
    Silent,
    /// Print each token exactly as it arrives. What `knaix chat` does, and the
    /// only shape safe to pipe: no escapes are introduced that were not in the
    /// answer.
    Raw,
...
/// them the answer went on to use.
///
/// Split out because the `meta` frame arrives before the first token: the
/// sources can be named while the model is still generating, which is the
/// difference between a spinner that says "Thinking" for the whole wait and one
/// that says what it found. `cited` is left false until the answer can settle
/// it.
...
///
/// Both stream shapes hand over the citations before the first token -- the
/// node through `onPrelude`, the control plane through its `meta` frame -- so
/// the wait for the model is time we can already say something about. It used
/// to read "Thinking..." from the moment the question was sent to the moment
/// the answer began; naming the documents turns the same wait into evidence
/// that retrieval found the right things, early enough to be worth reading.
