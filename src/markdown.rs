...
//!
//! Markdown is only meaningful in whole lines: `- ` is a bullet at the start of
//! one and a hyphen anywhere else. So a line is held until its newline arrives
//! and then rendered. Fenced code blocks are the exception, since their lines
//! mean nothing apart; an open fence buffers until it closes and goes out as
//! one block.
//!
//! Deciding what to render is separate from rendering it. `Segments` is a pure
//! state machine returning the pieces that have become renderable, and it is
