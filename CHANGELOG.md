...
The minor is for one behaviour change: where your default node is `local`, a bare `knaix list` now lists that node's documents rather than your hosted nodes, so anything scripted against it wants `--nodes`.

### Added

- **`knaix list` lists a local node's documents.** Name, chunk count, and when each was ingested. It asks the node directly, so it needs no account and no network, which is the situation you are usually in when you want it: a machine that cannot reach the control plane could not previously be asked what it was holding, and the error it gave named a DNS failure rather than anything you could act on.

- **`--docs` and `--nodes` say which listing you want.** One command returning two kinds of result, told apart by whether an argument happened to be present, is a guess dressed as an interface. Either flag makes it explicit and ignores the default, which is the form worth using in a script.

- **`knaix use local` now reaches `list` too.** It was the one command that never read the saved default, so setting it changed nothing about the command most likely to have just failed, and the advice printed on that failure told you to set it anyway.

### Changed

- **A bare `knaix list` follows a `local` default.** Every other command already treats the reserved name as addressing the node on this machine. With that default set, `knaix list` lists its documents; `--nodes` lists your hosted nodes as before. A hosted default is untouched and still lists nodes, so only a machine deliberately set to `local` sees the difference. This is why the release takes the minor.

- **`-o json` returns the same records for both kinds of node.** A local node's documents came back under the node's own field names, so a script written against a hosted node read nothing from a local one and said so with a null rather than an error. Local records now carry `id`, a `source` object, `chunkCount` and `createdAt`, as the control plane already reported them. A local node records no source type, so that field is null rather than inferred from the file extension.

### Fixed

- **`bench --sweep` and `selftest --sweep` work on a local node.** They reported removing nothing, because the enumeration they rest on returned an empty list for a local node while deleting by id had worked all along. The only remedy left on offer was `knaix local reset`, which empties the store: everything you had ingested, to clear one generated document. Both sweeps now find what they generated by name, and the message that pointed at `local reset` is gone.

- **The advice printed when the control plane is unreachable can be followed.** It offered `knaix use local` for a command that does not read the default, so you could run it, run the failing command again, and get the same error and the same note. It also picked its wording from the position of the subcommand, which a global flag moves, so `knaix -o json list` got the advice meant for other commands.

- **`knaix status` names a command that verifies your session.** It suggested `knaix list`, which with a local default never reaches the control plane, so a revoked token still read as saved. It names `knaix list --nodes`.

- **`--docs` with no default node set exits 7 rather than 2.** The command line is well formed; the machine is what is not ready, which is what 7 means and what naming a local node that was never started already returned.
